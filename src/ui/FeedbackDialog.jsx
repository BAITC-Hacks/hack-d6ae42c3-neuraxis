...
export default function FeedbackDialog({ title, children, onClose, onConfirm, confirmLabel = 'Продолжить', busy = false, autoCloseMs = 0 }) {
  const dialog = useRef(null);
  const close = useRef(onClose);
  close.current = onClose;
  useEffect(() => {
    if (!autoCloseMs || onConfirm || busy) return;
    const timeout = setTimeout(() => close.current(), autoCloseMs);
    return () => clearTimeout(timeout);
  }, [autoCloseMs, Boolean(onConfirm), busy]);
