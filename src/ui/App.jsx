...
export default function App() {
  ...
  const onProfile = person => {
    if (person?.id !== profile.id) {
      revision.current++; setSaved(false); setRuns([]);
  ...
  return <div className="app"><aside className="sidebar"><a className="brand" href="#" onClick={e => {
        e.preventDefault();
        setView('simulation');
      }}><span className="brand-icon"><Building2 size={24} /></span><span>QALA<span className="brand-light">LAB</span><small>ГОРОД НАЧИНАЕТСЯ С ВАС</small></span></a><div className="workspace-label">РАБОЧЕЕ ПРОСТРАНСТВО</div><nav>{[['simulation', Layers3, 'Симулятор'], ['analytics', BarChart3, 'Аналитика'], ['history', History, 'Мои сценарии']].map(([id, Icon, title]) => <button key={id} className={view === id ? 'nav-item active' : 'nav-item'} onClick={() => setView(id)}><Icon size={19} />{title}{id === 'simulation' ? <span className="nav-live" /> : id === 'history' && runs.length > 0 ? <small>{runs.length}</small> : null}</button>)}</nav><div className="sidebar-scenario"><span className="tiny-label">АКТИВНЫЙ СЦЕНАРИЙ</span><div className="scenario-art"><Building2 size={48} strokeWidth={1} /><span>ASTANA<br /><b>2026</b></span></div><h3>Город для жизни</h3><p>Три района. Пять направлений.<br />Ваше видение будущего.</p><span className="scenario-tag"><span /> Синтетические данные</span></div><div className="sidebar-bottom"><button className="help-link" onClick={() => setModal('help')}><Info size={18} /> Как устроена модель <ArrowUpRight size={15} /></button></div></aside>
  <div className="main-shell"><header className="topbar"><div className="breadcrumb">Рабочее пространство <ChevronRight size={13} /><b>{view === 'simulation' ? 'Симулятор города' : view === 'analytics' ? 'Аналитика сценария' : view === 'account' ? 'Личный кабинет' : 'Мои сценарии'}</b></div><div className="header-right"><span className="live-pill"><i /> Сессия активна</span><span className="locale">Астана</span><button className={`header-account ${view === 'account' ? 'active' : ''}`} onClick={() => setView('account')} aria-label="Личный кабинет"><span className="avatar">{(profile.nickname || 'Г')[0].toUpperCase()}</span><span className="header-account-copy"><b>{profile.id ? profile.team || profile.nickname : 'Личный кабинет'}</b><small>{profile.id ? 'Профиль и сохранения' : 'Войти / Зарегистрироваться'}</small></span><ChevronRight size={14} /></button></div></header><main><section className="page-heading"><div><div className="eyebrow"><span /> АСТАНА · ГОРОДСКАЯ ЛАБОРАТОРИЯ</div><h1>{view === 'account' ? <>Личный кабинет. <span>Ваша городская история.</span></> : view === 'simulation' ? <>Ваш город. <span>Ваши решения.</span></> : view === 'analytics' ? <>Решения, <span>которые меняют город.</span></> : <>Каждый сценарий — <span>новая возможность.</span></>}</h1><p>Аким на 5 часов: распределите бюджет и улучшите жизнь горожан.</p></div><button className="button secondary reset" onClick={() => {
            changeSelection({});
            setStep(0);
            setView('simulation');
          }}><RotateCcw size={15} /> Новый сценарий</button></section>
  {(error || notice) && <div role={error ? 'alert' : 'status'} className={`message ${error ? 'error' : ''}`}><Info size={17} /><span>{error || notice}</span><button aria-label="Закрыть уведомление" onClick={() => {
            setError('');
            setNotice('');
  ...
  {view === 'simulation' && <><section className="city-grid"><article className="panel map-panel"><div className="panel-heading"><div><span className="tiny-label">ГОРОД В ДЕТАЛЯХ</span><h2>Пульс Астаны <span className="live-dot" /></h2></div><span className="map-layer"><Layers3 size={14} /> {active.short}</span></div><CityMap districts={result.districts} active={districtId} onSelect={setDistrictId} selected={result.selected} dimension={active.id} /></article><article className="panel district-panel"><div className="panel-heading"><div><span className="tiny-label">В ФОКУСЕ</span><h2>Район {district.name}</h2></div><span className="soft-icon"><MapPin size={19} /></span></div><div className="district-tabs">{result.districts.map(d => <button key={d.id} className={d.id === districtId ? 'active' : ''} onClick={() => setDistrictId(d.id)}>{d.name}</button>)}</div><div className="population"><Users size={16} /><b>{district.population} тыс.</b> жителей <span>{district.zone}</span></div><div className="district-metrics">{scenario.dimensions.map(d => <div className="metric" key={d.id}><div><span>{d.short}</span><b>{number(district.projected[d.id])}<small className={district.projected[d.id] < district.stats[d.id] ? 'negative' : ''}>{delta(district.projected[d.id] - district.stats[d.id])}</small></b></div><div className="metric-track"><span style={{
              ...
              })}</div><div className="initiative-sources"><p>Инициативы на основе практик Астаны. Объём пилота, размещение в учебных районах, цены и эффекты — модельные.</p>{scenario.initiatives.filter(i => i.dimension === active.id).map(i => <a key={i.id} href={i.source.url} target="_blank" rel="noreferrer">{i.title} · официальный источник ↗</a>)}</div><div className="planning-footer"><span><Shield size={15} /> На остальные направления зарезервировано {money(result.reserved)}</span><div><button className="button secondary" disabled={step === 0} onClick={() => setStep(s => s - 1)} aria-label="Предыдущее направление"><ArrowLeft size={16} /></button><button className="button secondary" disabled={step === 4} onClick={() => setStep(s => s + 1)}>Следующее направление <ArrowRight size={16} /></button></div></div></section></>}
  {view === 'analytics' && <section className="analytics-grid"><article className="panel"><div className="panel-heading"><div><span className="tiny-label">ЭФФЕКТ ВАШИХ РЕШЕНИЙ</span><h2>Город: до и после</h2></div><BarChart3 size={22} /></div><div className="chart-legend"><span><i /> Исходное</span><span><i /> Ваш сценарий</span></div><div className="comparison-chart">{scenario.dimensions.map(d => <div className="comparison-row" key={d.id}><span>{d.label}</span><div><div className="chart-bar baseline" style={{
